        ...
        {isInThread && index > 0 && (() => {
          ...
          return (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "6px 0 2px" }}>
              <button onClick={(e) => { e.stopPropagation(); const p = siblings[(ci - 1 + siblings.length) % siblings.length]; setBranchSelections(s => ({ ...s, [parentId]: p.id })); setThreadViewId(p.id); }}
              ...
              <button onClick={(e) => { e.stopPropagation(); const n = siblings[(ci + 1) % siblings.length]; setBranchSelections(s => ({ ...s, [parentId]: n.id })); setThreadViewId(n.id); }}
                style={{ background: "transparent", border: "none", color: T.accent, fontSize: 22, cursor: "pointer", fontFamily: "'DM Mono', monospace", padding: "4px 12px", WebkitTapHighlightColor: "transparent" }}>›</button>
            </div>
          );
        })()}
